...
{
  uint8_t hash[64];
  uint8_t check_sum[64];
  uint8_t N[64];
  uint8_t data[64];
  size_t data_size;
  context(bool f = false) 
  {
    if (f) {
      for (int i = 0; i < 64; i++) {
        hash[i] = 1;
...
const uint8_t nonlinear_bi[256] ={252, 238, 221, 17, 207, 110, 49, 22, 251, 196, 250, 218, 35, 197, 4, 77, 233, 119, 240, 219, 147, 46,
...
const uint8_t tau[64]= {0,  8, 16, 24, 32, 40, 48, 56,
...
const uint64_t matrix_a[64]= {0x8e20faa72ba0b470, 0x47107ddd9b505a38, 0xad08b0e0c3282d1c, 0xd8045870ef14980e,
...
0x30, 0x02, 0xc6, 0xcd, 0x63, 0x5a, 0xfe,
0x94,
...
size_t data_read(FILE *f, context &ctx);

void add(uint8_t* data, uint8_t size);

void sum_512(uint8_t* tmp, uint8_t* tmp1);
